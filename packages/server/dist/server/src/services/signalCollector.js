...
async function computeAvailability(db, pairId, user1Id, user2Id) {
    const since = daysAgo(WINDOW_DAYS.AVAILABILITY);
    const rows = await db
        .select({ userId: messages.userId, count: sql `count(*)::int` })
        .from(messages)
...
async function computeAlignment(db, pairId, user1Id, user2Id) {
    ...
    const u1Starts = recentMessages.filter((m, i) => m.userId === user1Id && (i === 0 || recentMessages[i - 1].userId !== user1Id)).length;
    const u2Starts = recentMessages.filter((m, i) => m.userId === user2Id && (i === 0 || recentMessages[i - 1].userId !== user2Id)).length;
...
async function computeActivation(db, pairId, user1Id, user2Id) {
...
async function computeTrust(db, pairId, user1Id, user2Id) {
    ...
    async function countInWindow(from, to) {
        const rows = await db
            .select({ count: sql `count(*)::int` })
            .from(messages)
            .where(and(eq(messages.pairId, pairId), gte(messages.createdAt, from), sql `${messages.createdAt} < ${to}`));
    ...
    if (nearCount === 0 && midCount === 0 && farCount === 0)
        return 20;
    ...
    const journalRows = await db
        ...
        .limit(10);
    const bothJournaling = journalRows.some((j) => j.userId === user1Id) &&
        journalRows.some((j) => j.userId === user2Id);
    const journalBonus = bothJournaling ? 10 : 0;
    return clamp(consistencyScore + journalBonus);
}
// ─── Main export ─────────────────────────────────────────────────────────────
export async function collectSignalsAndDeriveState(db, pairId) {
    // Get pair membership
    ...
    const signals = { availability, alignment, activation, trust };
