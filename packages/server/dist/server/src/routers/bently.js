...
function buildBentlySystemPrompt(state, requestingUserId, partnerId, currentUserId) {
...
export const bentlyRouter = router({
    ...
    coach: publicProcedure
        ...
        const pair = await db.query.pairs.findFirst({
            where: and(eq(pairs.id, input.pairId), or(eq(pairs.user1Id, ctx.userId), eq(pairs.user2Id, ctx.userId)), eq(pairs.status, 'ACTIVE')),
        });
        if (!pair) {
        ...
        const relationalState = pair.relationalState;
        // Build state-aware system prompt
        const systemPrompt = buildBentlySystemPrompt(relationalState, ctx.userId, partnerId, ctx.userId);
        try {
...
Speak directly. Don't over-soften. Keep it under 150 words.`;
    ...
    // Placeholder stubs
    list: publicProcedure.query(() => ({ resource: 'bently', items: [] })),
    getById: publicProcedure
        .input(z.object({ id: z.string().min(1) }))
        .query(({ input }) => ({ resource: 'bently', id: input.id, item: null })),
});
