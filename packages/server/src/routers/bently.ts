...
// Full replacement — state-aware, dual-current, real AI calls.
...
// Bently's voice is constant. What changes per state is the behavioral directive
...
export const bentlyRouter = router({
  ...
  // ── Main coach endpoint ───────────────────────────────────────────────────
  // Primary Bently call. Requires an active pair. State-aware.
  coach: publicProcedure
    .input(z.object({
      pairId: z.string().uuid(),
      message: z.string().min(1).max(2000),
      provider: z.enum(['claude', 'venice']).default('claude'),
    }))
    .mutation(async ({ input, ctx }) => {
      ...
      await db
        ...
        .values({
          ...
          metadata: { responseId: saved.id, state: relationalState },
        });
  ...
  // For onboarding, pre-pair usage, or solo journaling mode.
  // Uses DORMANT state as the neutral default — enough to activate both currents
  // without assuming anything about relational health.
...
Speak directly. Don't over-soften. Keep it under 150 words.`;
  ...
  // ── Placeholder stubs (kept for router completeness) ──────────────────────
