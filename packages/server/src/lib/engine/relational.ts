import type { RelationalMetric, RelationalState } from '../../../../shared/enums.js';
import type { RelationalMetrics } from '../../../../shared/types.js';

const RELATIONAL_METRIC = {
  HIGH: 'HIGH',
  MEDIUM: 'MEDIUM',
  LOW: 'LOW',
} as const satisfies Record<string, RelationalMetric>;
...
} as const satisfies Record<string, RelationalState>;

export interface RelationalSignals {
  availability: number;
  alignment: number;
  activation: number;
  trust: number;
...
] as const satisfies readonly RelationalState[];

const clamp = (value: number): number => Math.max(0, Math.min(100, Math.round(value)));

export function toMetric(value: number): RelationalMetric {
  if (value >= 70) return RELATIONAL_METRIC.HIGH;
  if (value >= 40) return RELATIONAL_METRIC.MEDIUM;
  return RELATIONAL_METRIC.LOW;
}

export function evaluateRelationalState(signals: RelationalSignals): RelationalEvaluation {
...
function buildDimensions(signals: RelationalSignals): RelationalMetrics {
  return {
